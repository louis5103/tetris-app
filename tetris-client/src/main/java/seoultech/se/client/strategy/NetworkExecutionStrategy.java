...
import seoultech.se.backend.network.MultiPlayStrategies;
...
public class NetworkExecutionStrategy implements GameExecutionStrategy {
    private final MultiPlayStrategies multiPlayStrategies;

    /**
     * NetworkExecutionStrategy 생성자
     *
     * @param multiPlayStrategies 네트워크 통신을 담당하는 MultiPlayStrategies 인스턴스
     */
    public NetworkExecutionStrategy(MultiPlayStrategies multiPlayStrategies) {
        if (multiPlayStrategies == null) {
            throw new IllegalArgumentException("MultiPlayStrategies cannot be null");
        }
        this.multiPlayStrategies = multiPlayStrategies;
    }

    /**
     * 명령을 네트워크를 통해 실행
     *
     * MultiPlayStrategies.executeCommand()를 호출하여:
     * 1. 로컬에서 Client-side prediction 수행
     * 2. 서버에 명령 전송
     * 3. Predicted state를 즉시 반환
     *
     * 서버의 권위 있는 응답은 MultiPlayStrategies.onServerUpdate()에서
     * 비동기로 처리되어 상태를 조정합니다.
     *
     * @param command 실행할 게임 명령
     * @param currentState 현재 게임 상태 (MultiPlayStrategies 내부 상태 사용)
     * @return Client-side predicted 게임 상태
     */
    @Override
    public GameState execute(GameCommand command, GameState currentState) {
        // MultiPlayStrategies에 명령 전달
        // 내부적으로 Client-side prediction + 서버 전송 처리
        return multiPlayStrategies.executeCommand(command, currentState);
    }
}
